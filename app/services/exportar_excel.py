...
def generar_excel_horario_aula(clases: List[ClaseProgramada], nombre_aula: str) -> BytesIO:
    wb = Workbook()
    ws = wb.active
    ws.title = "Horario Aula"

    encabezado_font = Font(bold=True, color="FFFFFF")
    encabezado_fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
    center_align = Alignment(horizontal="center", vertical="center")

    headers = ["Materia", "Docente", "Día", "Hora Inicio", "Hora Fin"]
    ws.append(headers)
    for col_num, col in enumerate(headers, 1):
        cell = ws.cell(row=1, column=col_num)
        cell.font = encabezado_font
